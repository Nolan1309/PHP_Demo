...
if (isset($_GET['idAccount'])) {
    ...
    $AccountDetails = new admin();

    $acount = $AccountDetails->GetAccount($idAccount);
}
...
<div class="content-body">
    <div class="form-item">
        <form action="?View=customer-xuly" method="post" id="form-acccount">
            ...
            <input type="email" id="email" name="email" required value="<?php echo $acount["Email"]; ?>"><br>

            <label for="password">Mật khẩu:</label>
            <input type="password" id="password" name="password" required value="<?php echo $acount["MatKhau"]; ?>"><br>

            <label for="phone">Số điện thoại:</label>
            <input type="tel" id="phone" name="phone" required value="<?php echo $acount["Phone"]; ?>"><br>

            <label for="fullname">Họ và tên:</label>
            <input type="text" id="fullname" name="fullname" required value="<?php echo $acount["FullName"]; ?>"><br>
            ...
            <select name="level" id="level">
                <?php
                // Lặp qua từng lựa chọn và tạo option
                foreach (array(1 => 'Admin', 2 => 'User') as $value => $label) {
                    // Kiểm tra xem giá trị hiện tại có trùng khớp với giá trị từ cơ sở dữ liệu hay không
